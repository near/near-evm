...
pub fn sha3<H: Handler>(runtime: &mut Runtime) -> Control<H> {
    ...
    try_or_fail!(runtime.resize_offset(from, len));
...
pub fn extcodecopy<H: Handler>(runtime: &mut Runtime, handler: &H) -> Control<H> {
    ...
    try_or_fail!(runtime.resize_offset(memory_offset, len));
...
pub fn log<H: Handler>(runtime: &mut Runtime, n: u8, handler: &mut H) -> Control<H> {
    ...
    try_or_fail!(runtime.resize_offset(offset, len));
    let data = if len == U256::zero() {
        Vec::new()
    } else {
        ...
        runtime.get_memory(offset, len)
    };

    let mut topics = Vec::new();
    for _ in 0..(n as usize) {
        match runtime.stack_pop() {
            Ok(value) => {
                topics.push(value);
            }
            Err(e) => return Control::Exit(e.into()),
...
pub fn create<H: Handler>(runtime: &mut Runtime, is_create2: bool, handler: &mut H) -> Control<H> {
    ...
    try_or_fail!(runtime.resize_offset(code_offset, len));
    let code = if len == U256::zero() {
        ...
        runtime.get_memory(code_offset, len)
    };
    ...
    match handler.create(runtime.context.address, scheme, value, code) {
...
pub fn call<'config, H: Handler>(
    ...
    try_or_fail!(runtime.resize_offset(in_offset, in_len));
    try_or_fail!(runtime.resize_offset(out_offset, out_len));
    ...
    let input = if in_len == U256::zero() {
        ...
        runtime.get_memory(in_offset, in_len)
    };
